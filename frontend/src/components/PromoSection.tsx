...
const PromoCard = ({ 
  ...
  return (
    <div className="element-card overflow-hidden element-card-hover">
      {badge && (
        <div className="absolute top-2 right-2 bg-element-gold text-element-black px-3 py-1 rounded-full font-medium text-sm animate-pulse-soft">
          {badge}
        </div>
      )}
      ...
      <div className="p-4">
        ...
        <p className="text-element-black/80 text-sm mb-3">{description}</p>
        <div className="flex items-end justify-between">
          <div>
            <p className="text-element-black font-bold text-xl">{price}</p>
            {originalPrice && (
              <p className="text-element-black/60 text-sm line-through">{originalPrice}</p>
            )}
          </div>
          <Link 
            to="/promocoes" 
            className="bg-element-black text-white rounded-full p-2 hover:bg-element-gold hover:text-element-black transition-colors"
          >
...
const PromoSection = () => {
  return (
    <section className="element-section bg-element-white">
