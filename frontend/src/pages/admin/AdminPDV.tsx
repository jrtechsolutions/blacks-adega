...
const AdminPDV = () => {
  ...
  return (
    <AdminLayout noPadding={true}>
      <div className="flex flex-col h-[calc(100vh-4rem)] lg:h-screen bg-[#0B0B0B] w-full">
        ...
        <div className="flex-1 flex flex-col lg:flex-row gap-4 overflow-y-auto lg:overflow-hidden p-2 lg:p-4 pb-24 lg:pb-2">
          ...
          <div className="flex-1 flex flex-col gap-4">
            <div className="bg-[#1F1F1F] border border-[#2E2E2E] shadow-sm p-4 flex flex-col sm:flex-row justify-between items-center gap-2 rounded-lg">
              <div className="flex flex-col sm:flex-row items-center gap-2">
                <h1 className="text-xl font-medium text-element-black">PDV - Venda Local</h1>
                {activeSession && (
            ...
            <div className="flex-1 bg-[#1F1F1F] border border-[#2E2E2E] rounded-lg p-2 flex flex-col min-h-0">
              {/* Pinned Products */}
              <div className="mb-4 flex-shrink-0">
                <div className="flex gap-2 overflow-x-auto pb-2 md:grid md:grid-cols-3 xl:grid-cols-4 md:gap-2 md:overflow-x-visible">
                  {pinnedProducts.map(product => (
                    <button
                      key={product.id}
                      className="min-w-[220px] md:min-w-0 text-left p-3 border border-[#2E2E2E] rounded-md hover:border-[#D4AF37] transition-colors bg-[#1F1F1F] text-[#CFCFCF]"
        ...
        <div className="lg:hidden fixed bottom-0 left-0 right-0 z-20 flex flex-col items-center">
          <button
            className="bg-element-gold text-white rounded-t-lg shadow-lg flex items-center justify-center w-12 h-6"
          ...
          <div
            className={`w-full bg-[#141414] transition-all duration-300 overflow-hidden ${showActions ? 'max-h-[500px] p-2' : 'max-h-0'}`}
          >
            <div className="grid grid-cols-3 gap-2">
              {actionButtons.map(btn => btn.visible && (
                <Button 
                  key={btn.label}
                  variant="outline"
                  className="bg-[#D4AF37] hover:bg-[#E6C76A] text-[#0B0B0B] flex flex-col items-center justify-center py-3 h-auto"
                  onClick={btn.onClick}
                >
                  {btn.icon}
        ...
        <div className="hidden lg:grid bg-[#141414] p-2 grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2">
          {actionButtons.map(btn => btn.visible && (
            <Button 
              key={btn.label}
              variant="outline"
