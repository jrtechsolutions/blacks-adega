  ...
  return (
    <AdminLayout>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#FFFFFF]">Gerenciar Categorias</h1>
          <p className="text-[#CFCFCF] mt-2">
            Crie e gerencie as categorias dos produtos
          </p>
        </div>
        <Button onClick={openCreateDialog} className="flex items-center gap-2 bg-[#D4AF37] text-[#0B0B0B] hover:bg-[#E6C76A]">
      ...
      {/* Filtros */}
      <Card className="bg-[#1F1F1F] border border-[#2E2E2E] mb-6">
        <CardContent className="pt-6">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex items-center gap-2">
              <Filter className="h-4 w-4 text-[#CFCFCF]" />
              <select
                value={filterActive}
                onChange={(e) => setFilterActive(e.target.value as 'all' | 'active' | 'inactive')}
                className="border border-[#2E2E2E] rounded-md px-3 py-2 text-sm bg-[#1F1F1F] text-[#FFFFFF] focus:outline-none focus:ring-2 focus:ring-[#D4AF37]"
              >
                <option value="all" className="bg-[#1F1F1F] text-[#FFFFFF]">Todas</option>
                <option value="active" className="bg-[#1F1F1F] text-[#FFFFFF]">Ativas</option>
                <option value="inactive" className="bg-[#1F1F1F] text-[#FFFFFF]">Inativas</option>
              </select>
            </div>
          </div>
        </CardContent>
        ...
        <div className="grid gap-4">
          {filteredCategories.length === 0 ? (
            ...
            filteredCategories.map((category) => (
              <Card key={category.id} className="hover:shadow-md transition-shadow bg-[#1F1F1F] border border-[#2E2E2E]">
                <CardContent className="pt-6">
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3">
                        ...
                        <Badge variant={category.active ? "default" : "secondary"} className={category.active ? "bg-[#D4AF37] text-[#0B0B0B]" : "bg-gray-600 text-white"}>
                          {category.active ? "Ativa" : "Inativa"}
                        </Badge>
                        <Badge variant="outline" className="border-[#2E2E2E] text-[#CFCFCF]">
                          {category._count?.products || 0} produtos
                        </Badge>
                      </div>
                      {category.description && (
                        <p className="text-[#CFCFCF] mt-1">{category.description}</p>
                      )}
