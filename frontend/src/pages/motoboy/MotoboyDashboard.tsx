...
const MotoboyDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-element-gray">
      {/* Header fixo */}
      <header className="w-full bg-element-black text-white py-4 px-6 flex items-center justify-between shadow-md sticky top-0 z-10">
        <div className="flex items-center gap-3">
          <Truck className="h-7 w-7 text-element-gold" />
          <span className="font-semibold text-lg">Motoboy</span>
          {user?.name && <span className="ml-2 text-base font-normal text-element-gold">{user.name}</span>}
        </div>
        <Button variant="ghost" className="text-white hover:bg-element-gold/20" onClick={logout}>
          <LogOut className="h-5 w-5 mr-2" /> Sair
        </Button>
      </header>

      <main className="max-w-3xl mx-auto py-8 px-4">
        <h1 className="text-2xl font-bold mb-6 text-element-black">Minhas Entregas</h1>
        <div className="space-y-6">
          {orders.length === 0 ? (
            <Card className="p-8 text-center border-2 border-dashed border-element-gold bg-element-white/80">
              <Truck className="h-12 w-12 mx-auto text-element-gold mb-4" />
              <span className="text-gray-400">Nenhuma entrega atribuída no momento.</span>
            </Card>
          ) : (
            orders.map(order => (
              <Card key={order.id} className="overflow-hidden shadow-md border border-element-gold/30 bg-element-white">
                <CardHeader className="pb-2 flex flex-row items-center justify-between bg-element-gold/10">
                  <CardTitle className="text-lg flex items-center gap-2">
                    <Truck className="h-5 w-5 text-element-gold" />
                ...
                <CardContent>
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center mb-1 text-element-black">
                        <MapPin className="h-5 w-5 text-element-gold mr-2" />
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <div className="text-lg font-bold text-element-black">R$ {order.total.toFixed(2)}</div>
                      {order.status === 'DELIVERING' && (
                        <div className="flex gap-2">
                          <Button variant="outline" className="border-element-gold text-element-black hover:bg-element-gold/10" onClick={() => handleNavigate(order.address, order.id)}>
